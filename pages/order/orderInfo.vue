<template>
	<view>
		...
		<view class="goods-section">
			...
			<!-- 商品列表 -->
			<view class="g-item" v-for="(item,index) in orderInfo.subOrderInfoList" :key="index">
	...
	<!-- 	<view class="yt-list">
		...
		<view class="yt-list">
			...
			<view class="yt-list-cell b-b" v-if="orderInfo.score!=0">
				...
				<text class="cell-tip red">-￥{{orderInfo.score}}</text>
			</view>
			<view class="yt-list-cell b-b">
		...
		<view class="footer">
			...
			<text class="submit" @click="submit" v-if="orderInfo.state==1">去支付</text>
			<text class="submit send" @click="send" v-if="orderInfo.state==3">确认收货</text>
		</view>
...
<script>
	...
	export default {
		...
		onLoad(option){
			...
			this.addressData = this.orderInfo.orderAddressInfo
		},
		computed: {
